    ...
    while counter <= max_depth:
        for item in aux:
            for neighbor in graph.neighbors(item):
                if neighbor not in aux:
                    node2distances[neighbor] = counter
                    node2num_paths[neighbor] = 1
                    node2parents[neighbor] = item
                else:
                    if node2distances[neighbor] > node2distances[item]:
                        if item not in node2parents[neighbor]: 
                            node2parents[neighbor] = node2parents[neighbor] + item
                            node2num_paths[neighbor]= node2num_paths[neighbor] + 1 
                    
        for element in node2distances:
...
def partition_girvan_newman(graph, max_depth):
    ...
    g = nx.Graph()
    btwen = approximate_betweenness(new_graph, max_depth)
    cut = []
    components = []
    max_btw = 0 
    for key,value in btwen:
        if btwen[(key,value)] > max_btw: 
            max_btw = btwen[(key,value)]
            cut = (key,value)
    for key,value in btwen:
        g.add_node(cut[0])
        if key == cut[0] and value != cut[1]:
            if value in new_graph:
                new_graph.remove_node(value)
            g.add_node(value)
            g.add_edge(value,cut[0])
        elif value==cut[0] and key != cut[1]:
            if key in new_graph:  
                new_graph.remove_node(key)
            g.add_node(key)
            g.add_edge(key,cut[0])
    components.append(g)
    components.append(new_graph)
    return components
...
def get_subgraph(graph, min_degree):
    ...
    for key in degrees:
        if degrees[key] < min_degree:
            subgraph.remove_node(key)
    return subgraph
...
def score_max_depths(graph, max_depths):
    ...
    score = []
    for i in max_depths:
        partition = partition_girvan_newman(graph, i)
        cut = norm_cut(partition[0], partition[1], graph)
        score.append(i,cut)
    return score
...
def make_training_graph(graph, test_node, n):
    ...
    while n > 0:
        new_graph.remove_edge(test_node, neighbors[n-1])
        n -= 1
    return new_graph
...
def jaccard(graph, node, k):
    ...
    n = k
    while k > 0:
        jaccard_list.append(scores[n-k])
        k -= 1
    return jaccard_list
...
def path_score(graph, root, k, beta):
    ...
    max_depth = nx.number_of_nodes(graph)
    s = {}
    for node in graph.nodes():
        if node not in root:
            node2distances, node2num_paths, node2parents = bfs(graph, root, max_depth)
            s[(root, node)] = ((beta ** node2distances[node]) * node2num_paths[node])
            print(((beta ** node2distances[node]) * node2num_paths[node]))
    s = sorted(list(s.items()), key=lambda x: x[1], reverse=True)
    return s[:k]
...
def evaluate(predicted_edges, graph):
    ...
    ###TODO
    counter = 0
    for item in predicted_edges:
        if item[0] in graph.neighbors(item[1]):
            counter += 1
