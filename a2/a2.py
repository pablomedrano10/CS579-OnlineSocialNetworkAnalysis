...
def tokenize(doc, keep_internal_punct=False):
    ...
    if keep_internal_punct:
        strings = re.sub(r'[?|$|.|!|,|:|;|-|_|[|]|{|}|_]', r' ', doc.lower()).split()
    else:
        strings = re.sub('\W+', ' ', doc.lower()).split()
...
def token_pair_features(tokens, feats, k=4):
    """
    ...
    for i in range(0, len(tokens)-k+1):
        for j in range(i, i+k-1):
            for l in range(j+1, k+i):
                if ('token_pair='+tokens[j]+'__'+tokens[l]) not in feats.keys() and ('token_pair='+tokens[l]+'__'+tokens[j]) not in feats:
                    feats['token_pair='+tokens[j]+'__'+tokens[l]] = 1
                elif ('token_pair='+tokens[j]+'__'+tokens[l]) in feats.keys() or ('token_pair='+tokens[l]+'__'+tokens[j]) not in feats:
                    feats['token_pair='+tokens[j]+'__'+tokens[l]] += 1
    return
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for i in range(0, len(tokens_list)):
        token_features(tokens_list[i], feats)
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    punct = list(punct_vals)
    while punct:
        punc = punct.pop()
        tokens = [tokenize(doc, punc) for doc in docs]
        featu = list(feature_fns)
        for i in range(1,len(feature_fns)+1):
            for comb in combinations(feature_fns, i):
                min_fr = list(min_freqs)
                while min_fr:
                    freq = min_fr.pop()
                    matr, vocabulary = vectorize(tokens, comb, freq, vocab=None)
...
def mean_accuracy_per_setting(results):
    ...
    #                     if key2 != accuracy and key2 == key and value2 == value:
    #                         for item in list_means:
    #                             if value2 != item[1]:
    #                                 n += 1
    #                                 sum_accuracy += dic2[accuracy]
    #             list_means.append((sum_accuracy/n, value2))
    # list_means.sort(lambda x: x[1])
    #
    # return(list_means)
    #
    pass
...
def top_coefs(clf, label, n, vocab):
    ...
    for coeficient in top_coef:
        for term in top_coef_terms:
            coefs.append((term, coeficient))
