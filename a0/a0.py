...
consumer_key = '8S1T89itzcBmfqq9qmwzjS2Ui'
consumer_secret = 'ue4gntwgNiY6XjNy3PoKVoAWgCZimc5afiMhwAFEn2HHr6dNOw'
access_token = '459979519-ulJ2xEkFNndjAI6OBTHQIgtrwrEFJn6Qp7ydLNl2'
access_token_secret = 'wtoXFtb30XZ5XU8z5q1iu8CwC0UgHVWw3ZOFaHvCoSoO5'
...
def read_screen_names(filename):
    ...
    screen_names = my_file.read().splitlines()
    my_file.close()
    return screen_names
...
def get_users(twitter, screen_names):
    ...
    for name in screen_names:
        request = robust_request(twitter, 'users/lookup', {'screen_name' : name, 'count' : 5000}, 5)
        users = [item for item in request]
        user.append(users[0])
    return user
...
def get_friends(twitter, screen_name):
    ...
    ###TODO
    ids=[]
    request = robust_request(twitter, 'friends/ids', {'screen_name': screen_name, 'count':5000}, 5)
...
def add_all_friends(twitter, users):
    ...
    ###TODO
    for user in users:
        user.update(dict(friends = get_friends(twitter, user['screen_name'])))
    return
...
def friend_overlap(users):
    ...
    usuarios = users[:]
    overlap = []
    while len(usuarios) > 1:
        dic = usuarios.pop()
        n = 0
        for user in usuarios:
            for friend in dic['friends']:
                if friend in user['friends']:
                    n += 1
            overlap.append((user['screen_name'], dic['screen_name'], n))
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for user in users:
        if(user['screen_name'] == 'HillaryClinton'):
            hillary_friends = get_friends(twitter, 'HillaryClinton')
        if(user['screen_name'] == 'realDonaldTrump'):
            donald_friends = get_friends(twitter, 'realDonaldTrump')
    for friend in hillary_friends:
        if friend in donald_friends:
            commonfriend = friend
    request = robust_request(twitter, 'users/lookup', {'user_id' : commonfriend, 'count' : 5000}, 5)
    users = [item for item in request]
...
def create_graph(users, friend_counts):
    ...
    ###TODO
    graph = nx.DiGraph()
    friends_several = friend_counts.copy()
    friends_several = {ids: friends for ids, friends in friends_several.items() if friends > 1}
    for i in range(0,len(users)):
        graph.add_node(users[i]['screen_name'])
        for ID in friends_several: 
            for friends in users[i]['friends']:
                if ID == friends:
                    graph.add_edge(ID, users[i]['screen_name'])
    return graph
